                                           ...
                                           <?php $i = 1; foreach ($service as $dt): ?>
                                                   <tr>
                                                       ...
                                                       <td class="actions">
                                                           ...
                                                           <a href="<?php echo $this->Url->build(["action" => "reviewview", $dt->id,$id]); ?>"> <button class="btn-btn-info btn-xs"><i class="icon-eye-open"></i> View</button> </a>
                                                           <a href="<?php echo $this->Url->build(["action" => "serviceprovider_editreview", $dt->id,$id]); ?>"> <button class="btn btn-primary btn-xs"><i class="icon-pencil icon-white"></i> Edit</button>  </a>
                                                           <a href="<?php echo $this->Url->build(["action" => "serviceprovider_reviewdelete", $dt->id],$id); ?>" onclick="return confirm('Are you sure you want to delete?');"> <button class="btn btn-danger btn-xs"><i class="icon-remove icon-white"></i> Delete</button> </a>                
                                                            <?php if ($dt->is_active == 1) { ?>
                                                                <a href="<?php echo $this->Url->build(["action" => "serviceprovider_reviewstatus", $dt->id, '0',$id]); ?>"> <button class="btn btn-info btn-xs"><i class="icon-thumbs-down"></i> Suspend</button> </a>
                                                            <?php } else if ($dt->is_active == 0) { ?>
                                                                <a href="<?php echo $this->Url->build(["action" => "serviceprovider_reviewstatus", $dt->id, '1',$id]); ?>"> <button class="btn btn-success btn-xs"><i class="icon-thumbs-up"></i> Approve</button> </a>
                                                            <?php } ?>
